...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        <a href="{{ route('admin.orders.index') }}" style="font-size:13px; color:var(--accent); text-decoration:none; font-weight:600; display:inline-flex; align-items:center; gap:5px; margin-bottom:6px;">&larr; Back to orders</a>
        <h2 style="font-size:22px; font-weight:700; color:#0F172A; margin:0;">Edit Order</h2>
        <p style="font-size:13px; color:#64748B; margin:4px 0 0;">Update order details for Order #{{ $order->id }}.</p>
    </div>
...
<form method="POST" action="{{ route('admin.orders.update', $order) }}">
    ...
    <div class="form-card">
        <div class="form-card-header">
            <div class="form-card-icon"><i class="fas fa-shopping-cart"></i></div>
            <div>
                <p style="font-size:14px; font-weight:700; color:#0F172A; margin:0;">Order Information</p>
                <p style="font-size:12px; color:#94A3B8; margin:0;">Update order details</p>
            </div>
        </div>
        <div class="form-card-body">
            <div class="field-group">
                <label class="field-label" for="status">Status <span class="req">*</span></label>
                <select name="status" id="status" class="field-select {{ $errors->has('status') ? 'error' : '' }}" required>
    ...
    <div style="display:flex; justify-content:flex-end; gap:12px; margin-top:18px;">
        ...
        <button type="submit" class="btn-primary"><i class="fas fa-save"></i> Update Order</button>
    </div>
</form>
