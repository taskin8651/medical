...
<div style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:24px; flex-wrap:wrap;">
    <div>
        ...
        <h2 style="font-size:22px; font-weight:700; margin:8px 0 0; color:#0F172A;">Edit Hero Slide</h2>
        <p style="font-size:13px; color:#64748B; margin:4px 0 0;">Update the hero details.</p>
    </div>
</div>
...
<form method="POST" action="{{ route('admin.hero.update', $hero) }}" enctype="multipart/form-data">
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div>
                ...
                <p style="font-size:12px; color:#94A3B8; margin:0;">Modify hero content and image.</p>
            </div>
        </div>
        <div class="form-card-body">
            <div class="field-group">
                <label class="field-label" for="title">Title <span style="color:#EF4444;">*</span></label>
                <input type="text" name="title" id="title" value="{{ old('title', $hero->title) }}" class="field-input">
            ...
            <div class="field-group">
                ...
                <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $hero->subtitle) }}" class="field-input">
            </div>
            <div class="field-group">
                <label class="field-label" for="description">Description</label>
                <textarea name="description" id="description" class="field-textarea">{{ old('description', $hero->description) }}</textarea>
            </div>
            <div class="field-group">
                <label class="field-label" for="button_text">Button Text</label>
                <input type="text" name="button_text" id="button_text" value="{{ old('button_text', $hero->button_text) }}" class="field-input">
            </div>
            <div class="field-group">
                <label class="field-label" for="button_link">Button Link</label>
                <input type="text" name="button_link" id="button_link" value="{{ old('button_link', $hero->button_link) }}" class="field-input">
            </div>
            <div class="field-group">
                <label class="field-label" for="image">Hero Image</label>
                <input type="file" name="image" id="image" class="field-input">
                @if($hero->image)
                <p style="font-size:12px; color:#475569; margin-top:8px;">Current: <img src="{{ asset('storage/' . $hero->image) }}" alt="Hero" style="max-height:60px; margin-top:6px; display:block; border-radius:10px;"></p>
                @endif
            ...
            <div class="field-group">
                ...
                <select name="status" id="status" class="field-select">
                    <option value="1" {{ old('status', $hero->status) == 1 ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ old('status', $hero->status) == 0 ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div style="display:flex; justify-content:flex-end; gap:12px; margin-top:18px;">
        ...
        <button type="submit" class="btn-primary"><i class="fas fa-save"></i> Update Hero</button>
    </div>
</form>
@endsection