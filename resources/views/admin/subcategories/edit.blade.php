...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        <a href="{{ route('admin.subcategories.index') }}" style="font-size:13px; color:var(--accent); text-decoration:none; font-weight:600; display:inline-flex; align-items:center; gap:5px; margin-bottom:6px;">&larr; Back to subcategories</a>
        <h2 style="font-size:22px; font-weight:700; color:#0F172A; margin:0;">Edit Subcategory</h2>
        <p style="font-size:13px; color:#64748B; margin:4px 0 0;">Update subcategory details for {{ $subcategory->name }}.</p>
    </div>
    <div style="display:flex; align-items:center; gap:10px; padding:12px 16px; background:#fff; border:1px solid #E2E8F0; border-radius:12px;">
        <div style="width:38px; height:38px; border-radius:12px; background:#E0E7FF; color:#3730A3; display:flex; align-items:center; justify-content:center; font-size:16px; font-weight:700;">{{ strtoupper(substr($subcategory->name, 0, 1)) }}</div>
        <div>
            <p style="font-size:14px; font-weight:700; margin:0; color:#0F172A;">{{ $subcategory->name }}</p>
            <p style="font-size:12px; color:#64748B; margin:0;">ID #{{ $subcategory->id }}</p>
        </div>
    </div>
</div>
...
<form method="POST" action="{{ route('admin.subcategories.update', $subcategory) }}">
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div>
                ...
                <p style="font-size:12px; color:#94A3B8; margin:0;">Update subcategory details</p>
            </div>
        </div>
        <div class="form-card-body">
            <div class="field-group">
                <label class="field-label" for="name">Name <span class="req">*</span></label>
                <input type="text" name="name" id="name" value="{{ old('name', $subcategory->name) }}" class="field-input {{ $errors->has('name') ? 'error' : '' }}" required>
    ...
    <div style="display:flex; justify-content:flex-end; gap:12px; margin-top:18px;">
        ...
        <button type="submit" class="btn-primary"><i class="fas fa-save"></i> Update Subcategory</button>
    </div>
</form>
