...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        <a href="{{ route('admin.categories.index') }}" style="font-size:13px; color:var(--accent); text-decoration:none; font-weight:600; display:inline-flex; align-items:center; gap:5px; margin-bottom:6px;">&larr; Back to categories</a>
        <h2 style="font-size:22px; font-weight:700; color:#0F172A; margin:0;">Edit Category</h2>
        <p style="font-size:13px; color:#64748B; margin:4px 0 0;">Update category details for {{ $category->name }}.</p>
    </div>
    <div style="display:flex; align-items:center; gap:10px; padding:12px 16px; background:#fff; border:1px solid #E2E8F0; border-radius:12px;">
        <div style="width:38px; height:38px; border-radius:12px; background:#E0E7FF; color:#3730A3; display:flex; align-items:center; justify-content:center; font-size:16px; font-weight:700;">{{ strtoupper(substr($category->name, 0, 1)) }}</div>
        <div>
            <p style="font-size:14px; font-weight:700; margin:0; color:#0F172A;">{{ $category->name }}</p>
            <p style="font-size:12px; color:#64748B; margin:0;">ID #{{ $category->id }}</p>
        </div>
    </div>
</div>
...
<form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data">    @method('PUT')
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div>
                <p style="font-size:14px; font-weight:700; color:#0F172A; margin:0;">Category Information</p>
                <p style="font-size:12px; color:#94A3B8; margin:0;">Update category details</p>
            </div>
        </div>
